...
namespace TarodevController {
    ...
    public class PlayerController : MonoBehaviour, IPlayerController {
        ...
        public virtual void ApplyVelocity(Vector2 vel, PlayerForce forceType) {
            ...
            else if (forceType == PlayerForce.Decay) _currentExternalVelocity += vel;
            else {
                _speed = vel;
        ...
        protected virtual void GatherInput() {
            ...
            if (_frameInput.SaveDown) GetComponent<PlayerSaveLoadController>().Save();
            if (_frameInput.LoadDown) GetComponent<PlayerSaveLoadController>().Load();
        }
        ...
        protected virtual void HandleJump() {
            if ((_jumpToConsume || HasBufferedJump) && _hasControl) {
        ...
        protected virtual void HandleHorizontal() {
            ...
            if (_wallHitCount > 0 &&                                                                 // Against a wall
                Mathf.Approximately(_rb.velocity.x, 0) &&                                            // RB velocity 0
                (Mathf.Sign(_frameInput.Move.x) == Mathf.Sign(_speed.x) || _frameInput.Move.x == 0)) // Moving towards the wall or not moving
                _speed.x = 0;
            
            if (_frameInput.Move.x != 0 && _hasControl) {
                if (_crouching && _grounded) {
                    ...
                    var targetSpeed = inputX * _stats.MaxSpeed;
                    if (!_grounded) targetSpeed = inputX > 0 ? Mathf.Max(targetSpeed, _speed.x) : Mathf.Min(targetSpeed, _speed.x);
                    _speed.x = Mathf.MoveTowards(_speed.x, targetSpeed, _currentWallJumpMoveMultiplier * _stats.Acceleration * Time.fixedDeltaTime);
                }
            }
            else if (_hasControl)
        ...
        protected virtual void HandleVertical() {
            ...
            // In Air
            var fallSpeed = Gravity ? _stats.FallAcceleration : 0f;
